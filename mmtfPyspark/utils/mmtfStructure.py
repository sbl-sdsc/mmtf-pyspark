...
class MmtfStructure(object):
    ...
    def __init__(self, input_data):
        ...
        self.group_id_list = np.cumsum(mmtfDecoder.run_length_decoder_numpy(
            np.frombuffer(input_data[b'groupIdList'][12:], '>i4'))).astype(np.int32)
        self.group_type_list = np.frombuffer(
            input_data[b'groupTypeList'][12:], '>i4')
        self.chain_id_list = [chr(a)
                              for a in input_data[b"chainIdList"][12:][::4]]
        self.group_list = mmtfDecoder.decode_group_list(
            input_data[b'groupList'])
        self.x_coord_list = mmtfDecoder.recursive_index_decode(np.frombuffer(
            input_data[b'xCoordList'][12:], '>i2'), np.frombuffer(input_data[b'xCoordList'][8:12], '>i'))
        self.y_coord_list = mmtfDecoder.recursive_index_decode(np.frombuffer(
            input_data[b'yCoordList'][12:], '>i2'), np.frombuffer(input_data[b'yCoordList'][8:12], '>i'))
        self.z_coord_list = mmtfDecoder.recursive_index_decode(np.frombuffer(
            input_data[b'zCoordList'][12:], '>i2'), np.frombuffer(input_data[b'xCoordList'][8:12], '>i'))
